  ...
  if (project.coverImage) {
    return (
      <div className={`relative overflow-hidden bg-[#17191b] ${compact ? "h-64" : "min-h-[400px]"}`}>
        ...
        <div className="absolute bottom-4 start-4 rounded-full border border-white/15 bg-black/35 px-3 py-1.5 text-[9px] font-extrabold uppercase tracking-[.16em] text-white/85 backdrop-blur-md">
          Product case study
        </div>
      </div>
    );
      ...
      <div className={`relative overflow-hidden ${compact ? "h-64" : "min-h-[360px]"}`}>
        ...
        <div className="absolute inset-x-5 top-6 rounded-2xl border border-white/10 bg-[#080e28]/95 p-4 shadow-2xl">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[['148','Shipments'],['23','On route'],['11','Exceptions']].map(([n,l])=><div key={l} className={`${common} p-3`}><div className="text-xl font-extrabold text-white">{n}</div><div className="mt-1 text-[9px] uppercase tracking-wider text-slate-500">{l}</div></div>)}
          </div>
