...
export function ProjectVisual({ project, compact = false }: { project: PortfolioItem; compact?: boolean }) {
  ...
  if (project.coverImage) {
    return (
      <div className={`relative overflow-hidden bg-[#070b1e] ${compact ? "h-60 sm:h-64" : "min-h-[380px]"}`}>
        <Image
          ...
          priority={!compact}
        />
        <div className="absolute inset-0 bg-gradient-to-t from-[#080d24] via-transparent to-transparent opacity-85" />
        <div className="absolute top-3.5 end-3.5 rounded-full border border-cyan-400/20 bg-black/60 px-3 py-1 text-[10px] font-bold text-cyan-200 backdrop-blur-md">
          {project.category === "platform" ? "Web Platform" : project.category === "business" ? "Business System" : "Mobile App"}
        </div>
      </div>
    );
  }
      ...
      <div className={`relative overflow-hidden ${compact ? "h-64" : "min-h-[360px]"}`}>
        ...
        <div className="absolute inset-x-5 top-6 rounded-2xl border border-white/10 bg-[#080e28]/95 p-4 shadow-2xl">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[['148','Shipments'],['23','On route'],['11','Exceptions']].map(([n,l])=><div key={l} className={`${common} p-3`}><div className="text-xl font-bold text-white">{n}</div><div className="mt-1 text-[9px] uppercase tracking-wider text-slate-500">{l}</div></div>)}
          </div>
